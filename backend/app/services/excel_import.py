...
HEADER_ALIASES = {
    ...
    "lcsc_number": ["物料编号", "商品编号", "立创编号", "LCSC", "LCSC编号", "产品编号"],
...
def guess_category(name: str, model: str | None, parameters: str | None, db: Session) -> int | None:
    return None
...
def parse_excel(content: bytes, db: Session, filename: str | None = None) -> list[ParsedRow]:
    ...
    for row_idx, values in raw_rows:
        ...
        data = normalize_component_values({
            ...
            "model": model or None,
            "category_id": guess_category(" ".join([name, product_type]), model, parameters, db),
            "parameters": parameters or None,
            "package": package or None,
            "quantity": quantity,
            "source": _pick(values, mapping, "source") or "立创商城 Excel",
...
def create_import_record(db: Session, row: dict[str, Any], component_id: int | None):
    order_number = row.get("order_number")
    lcsc_number = row.get("lcsc_number")
